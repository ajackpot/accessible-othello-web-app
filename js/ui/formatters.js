...
export function formatResolvedOptionsList(options) {
  ...
  return [
    ...
    ...(isMctsMode || !options.classicEngineVariantLabel
      ? []
      : [{ label: '클래식 읽기 성향', value: options.classicEngineVariantLabel }]),
...
export function formatEngineSummaryLine(options) {
  ...
  const styleMainLaneSuffix = doesSearchAlgorithmUseStyleEvaluator(options.searchAlgorithm)
    ? ''
    : ' · 메인 탐색 스타일 미적용';
  const classicVariantSuffix = (!isMctsSearchAlgorithm(options.searchAlgorithm) && options.classicEngineVariantSummaryLabel)
    ? ` · ${options.classicEngineVariantSummaryLabel}`
    : '';

  if (isMctsSearchAlgorithm(options.searchAlgorithm)) {
    return `${searchAlgorithm?.summaryLabel ?? searchAlgorithm?.label ?? 'MCTS'} · ${presetLabel} · ${styleLabel} · 제한 ${options.timeLimitMs}ms · 탐험 ${options.mctsExploration ?? ''} · 후반 완전 탐색 ${options.exactEndgameEmpties}칸 이하 · 사전 WLD ${formatWldPreExactSetting(options.wldPreExactEmpties, { compact: true })}${styleMainLaneSuffix}${classicVariantSuffix}`;
  }

  return `${searchAlgorithm?.summaryLabel ?? searchAlgorithm?.label ?? '클래식'} · ${presetLabel} · ${styleLabel} · 깊이 ${options.maxDepth} · 제한 ${options.timeLimitMs}ms · 후반 완전 탐색 ${options.exactEndgameEmpties}칸 이하 · 사전 WLD ${formatWldPreExactSetting(options.wldPreExactEmpties, { compact: true })}${styleMainLaneSuffix}${classicVariantSuffix}`;
}
