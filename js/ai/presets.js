...
import {
  CLASSIC_ENGINE_VARIANT_OPTIONS,
...
export const CUSTOM_DIFFICULTY_CLASSIC_ONLY_FIELDS = Object.freeze([
  ...
  {
    key: 'classicEngineVariant',
    label: '클래식 읽기 성향',
    type: 'select',
    options: CLASSIC_ENGINE_VARIANT_OPTIONS,
    helpText: '기본값은 표준 읽기입니다. 확장 후보형은 후보폭을 조금 더 넓히고 양방향 컷까지 함께 사용합니다.',
  },
]);
...
function buildCustomDifficultyBaseDefaults(baseInputs = {}) {
  ...
  const baseFields = [
    ...LEGACY_CUSTOM_DIFFICULTY_FIELDS,
    ...CUSTOM_DIFFICULTY_CLASSIC_ONLY_FIELDS.filter((field) => !LEGACY_CUSTOM_DIFFICULTY_FIELDS.some((legacyField) => legacyField.key === field.key)),
  ];
