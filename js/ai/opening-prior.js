...
const OPENING_PRIOR_FACTORIZED_FORMAT = 'factorized-binary-varint-v1';
const FACTORIZED_OPENING_PRIOR_CACHE = new WeakMap();
...
function readUnsignedVarint(bytes, startOffset) {
  ...
  throw new Error('Unexpected end of factorized opening prior varint stream.');
}

function readSignedVarint(bytes, startOffset) {
  const decoded = readUnsignedVarint(bytes, startOffset);
  const value = decoded.value % 2 === 0
    ? decoded.value / 2
    : -((decoded.value + 1) / 2);
...
function isFactorizedOpeningPriorProfile(profile) {
  return typeof profile?.format === 'string'
    && profile.format.trim().toLowerCase() === OPENING_PRIOR_FACTORIZED_FORMAT;
}

function hashBytesToString(hashBytes, hashEncoding = 'hex') {
  let hex = '';
  for (const byte of hashBytes) {
    hex += byte.toString(16).padStart(2, '0');
  }
  const normalizedHex = hex.replace(/^0+/, '') || '0';
  if (normalizeHashEncoding(hashEncoding, 'hex') === 'hex') {
    return normalizedHex;
  }
  return BigInt(`0x${normalizedHex}`).toString(10);
}

function materializeFactorizedOpeningPrior(profile, header) {
  const cached = FACTORIZED_OPENING_PRIOR_CACHE.get(profile) ?? null;
  if (cached) {
    return cached;
  }

  const factorized = profile?.factorized && typeof profile.factorized === 'object' ? profile.factorized : {};
  const hashBytes = decodeBase64Bytes(factorized.hashBytesBase64);
  const hashLengths = decodeBase64Bytes(factorized.hashLengthsBase64);
  const plys = decodeBase64Bytes(factorized.plysBase64);
  const candidateCounts = decodeBase64Bytes(factorized.candidateCountsBase64);
  const totalCountBytes = decodeBase64Bytes(factorized.totalCountsBase64);
  const moveIndices = decodeBase64Bytes(factorized.moveIndicesBase64);
  const moveCountBytes = decodeBase64Bytes(factorized.moveCountsBase64);
  const moveScoreBytes = decodeBase64Bytes(factorized.moveScoresBase64);

  const positionCount = hashLengths.length;
  if (plys.length !== positionCount || candidateCounts.length !== positionCount) {
    throw new Error('Factorized opening prior metadata length mismatch.');
  }

  let hashOffset = 0;
  let totalOffset = 0;
  let moveIndexOffset = 0;
  let moveCountOffset = 0;
  let moveScoreOffset = 0;
  const rawPositions = [];
  const positionsByKey = new Map();

  for (let positionIndex = 0; positionIndex < positionCount; positionIndex += 1) {
    const hashLength = hashLengths[positionIndex];
    const nextHashOffset = hashOffset + hashLength;
    if (nextHashOffset > hashBytes.length) {
      throw new Error('Factorized opening prior hash payload ended unexpectedly.');
    }
    const stateHash = hashBytesToString(hashBytes.subarray(hashOffset, nextHashOffset), header.hashEncoding);
    hashOffset = nextHashOffset;

    const totalCountDecoded = readUnsignedVarint(totalCountBytes, totalOffset);
    totalOffset = totalCountDecoded.nextOffset;
    const rawPosition = [stateHash, plys[positionIndex] ?? 0, totalCountDecoded.value];
    const candidateCount = candidateCounts[positionIndex] ?? 0;
    for (let candidateIndex = 0; candidateIndex < candidateCount; candidateIndex += 1) {
      if (moveIndexOffset >= moveIndices.length) {
        throw new Error('Factorized opening prior move index payload ended unexpectedly.');
      }
      const moveCountDecoded = readUnsignedVarint(moveCountBytes, moveCountOffset);
      moveCountOffset = moveCountDecoded.nextOffset;
      const moveScoreDecoded = readSignedVarint(moveScoreBytes, moveScoreOffset);
      moveScoreOffset = moveScoreDecoded.nextOffset;
      rawPosition.push(moveIndices[moveIndexOffset], moveCountDecoded.value, moveScoreDecoded.value);
      moveIndexOffset += 1;
    }

    rawPositions.push(rawPosition);
    positionsByKey.set(normalizeHashString(stateHash, header.hashEncoding), rawPosition);
  }

  if (hashOffset !== hashBytes.length || totalOffset !== totalCountBytes.length || moveIndexOffset !== moveIndices.length || moveCountOffset !== moveCountBytes.length || moveScoreOffset !== moveScoreBytes.length) {
    throw new Error('Factorized opening prior payload has unread bytes after decode.');
  }

  const materialized = {
    rawPositions: Object.freeze(rawPositions.map((position) => Object.freeze([...position]))),
    positionsByKey,
  };
  FACTORIZED_OPENING_PRIOR_CACHE.set(profile, materialized);
  return materialized;
}
...
function resolveOpeningPriorProfileHeader(profile) {
  ...
  return {
    ...
    symmetry: typeof profile?.symmetry === 'string' && profile.symmetry.trim() !== ''
      ...
      : 'canonical-4',
    format: isFactorizedOpeningPriorProfile(profile)
      ? OPENING_PRIOR_FACTORIZED_FORMAT
      : (isCompactOpeningPriorProfile(profile) ? OPENING_PRIOR_COMPACT_FORMAT : 'expanded-v1'),
    hashEncoding: normalizeHashEncoding(
      profile?.hashEncoding,
      (isCompactOpeningPriorProfile(profile) || isFactorizedOpeningPriorProfile(profile)) ? 'hex' : 'decimal',
    ),
...
export const RUNTIME_OPENING_PRIOR_PROFILE = GENERATED_OPENING_PRIOR_PROFILE ?? null;

export function resolveOpeningPriorProfile(profile = RUNTIME_OPENING_PRIOR_PROFILE) {
  if (!profile) {
    return null;
  }
  ...
  const rawPositions = isFactorizedOpeningPriorProfile(profile)
    ? materializeFactorizedOpeningPrior(profile, header).rawPositions
...
let cachedRuntimeCompiledProfile = null;

export function compileOpeningPriorProfile(profile = RUNTIME_OPENING_PRIOR_PROFILE) {
  const header = resolveOpeningPriorProfileHeader(profile);
  if (!header) {
    return null;
  }

  if (isFactorizedOpeningPriorProfile(profile)) {
    const materialized = materializeFactorizedOpeningPrior(profile, header);
    return {
      ...header,
      positions: materialized.rawPositions,
      positionsByKey: materialized.positionsByKey,
...
export function lookupOpeningPrior(state, profile = RUNTIME_OPENING_PRIOR_PROFILE) {
  if (!(state instanceof GameState)) {
    throw new TypeError('lookupOpeningPrior expects a GameState instance.');
  }

  const compiled = profile === RUNTIME_OPENING_PRIOR_PROFILE
    ? (cachedRuntimeCompiledProfile ??= compileOpeningPriorProfile(profile))
