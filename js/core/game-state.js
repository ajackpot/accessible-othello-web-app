...
import {
  ...
  applyMoveBitWithPreparedSearchMoveToken,
  getDiscCounts,
  getInitialBoards,
  isTerminalPosition,
...
function createFastGameState(
  ...
  moveHistory,
  lastAction,
  emptyBitboard = undefined,
  emptyCount = undefined,
  discCounts = undefined,
  hashKey = undefined,
) {
  ...
  state._emptyBitboard = emptyBitboard;
  state._emptyCount = emptyCount;
  state._discCounts = discCounts;
  state._hashKey = hashKey;
  return state;
}
...
export class GameState {
  ...
  createSearchChildStateFromPlayerBoards(
    nextPlayerBoard,
    nextOpponentBoard,
    moveIndex = null,
    flipCount = null,
    moveBitOverride = null,
  ) {
    const color = this.currentPlayer;
    const opponentColor = this.getOpponentColor(color);
    const nextBlack = color === PLAYER_COLORS.BLACK ? nextPlayerBoard : nextOpponentBoard;
    const nextWhite = color === PLAYER_COLORS.WHITE ? nextPlayerBoard : nextOpponentBoard;
    const moveBit = typeof moveBitOverride === 'bigint'
      ? moveBitOverride
      : (Number.isInteger(moveIndex) && moveIndex >= 0 && moveIndex < 64
        ? bitFromIndex(moveIndex)
        : 0n);
    const nextEmptyBitboard = this._emptyBitboard !== undefined && moveBit !== 0n
      ? this._emptyBitboard & ~moveBit
      : undefined;
    const nextEmptyCount = this._emptyCount !== undefined && moveBit !== 0n
      ? this._emptyCount - 1
      : undefined;

    let nextDiscCounts;
    if (this._discCounts !== undefined && Number.isInteger(flipCount)) {
      if (color === PLAYER_COLORS.BLACK) {
        nextDiscCounts = {
          black: this._discCounts.black + 1 + flipCount,
          white: this._discCounts.white - flipCount,
        };
      } else {
        nextDiscCounts = {
          black: this._discCounts.black - flipCount,
          white: this._discCounts.white + 1 + flipCount,
        };
      }
    }

    const nextHashKey = nextBlack
      | (nextWhite << HASH_WHITE_SHIFT)
      | (opponentColor === PLAYER_COLORS.WHITE ? HASH_WHITE_TO_MOVE_BIT : 0n);

    return createFastGameState(
      nextBlack,
      nextWhite,
      opponentColor,
      0,
  ...
  applyMoveFast(index, precomputedFlips = null) {
    ...
    const opponentColor = this.getOpponentColor(color);
    const { player, opponent } = this.getPlayerBoards(color);
    const result = typeof precomputedFlips === 'bigint'
      ? applyMoveBitWithFlips(moveBit, precomputedFlips, player, opponent)
      : (Number.isInteger(precomputedFlips)
        ? applyMoveBitWithPreparedSearchMoveToken(index, precomputedFlips, player, opponent)
