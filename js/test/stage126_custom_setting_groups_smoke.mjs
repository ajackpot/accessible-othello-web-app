...
assert.equal(defaultDifficultyInputs.classicEngineVariant, 'stage154-standard', 'default custom difficulty should install the standard classic runtime variant');
...
assert.ok(!Object.hasOwn(defaultStyleInputs, 'edgePatternScale'), 'retired handcrafted edge-pattern scale should not surface in custom style defaults');
assert.ok(!Object.hasOwn(defaultStyleInputs, 'cornerPatternScale'), 'retired handcrafted corner-pattern scale should not surface in custom style defaults');
...
assert.ok(classicFieldKeys.includes('classicEngineVariant'), 'classic custom difficulty should expose the classic runtime variant selector');
...
assert.ok(!guidedFieldKeys.includes('classicEngineVariant'), 'guided custom difficulty should hide the classic runtime variant selector');
