...
import { RUNTIME_MPC_PROFILE } from '../ai/evaluation-profiles.js';
...
const RUNTIME_MPC_NAME = RUNTIME_MPC_PROFILE?.name ?? null;
assert.ok(RUNTIME_MPC_NAME, 'An installed runtime MPC profile should be present before the default parity smoke runs.');
...
assert.equal(
  ...
  RUNTIME_MPC_NAME,
  'Bare SearchEngine construction should inherit the installed runtime MPC profile by default.',
);
...
assert.equal(
  ...
  RUNTIME_MPC_NAME,
  'Direct SearchEngine calls should expose the installed runtime MPC profile in the result snapshot by default.',
);
assert.ok(
...
assert.equal(
  ...
  RUNTIME_MPC_NAME,
  'benchmark-helpers should now report the installed runtime MPC profile when no explicit override is given.',
);
assert.ok(
...
assert.equal(
  ...
  RUNTIME_MPC_NAME,
  'EngineClient fallback should still report the installed runtime MPC profile.',
);
assert.equal(
...
console.log('stage121_runtime_mpc_default_parity_smoke: all assertions passed');
