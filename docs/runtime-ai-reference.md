...
| runtime evaluation profile | `balanced13-alllate-smoothed stability extras 0.90x` | `js/ai/learned-eval-profile.generated.js`, `js/ai/evaluation-profiles.js` |
| runtime move-ordering profile | `stage154-main-recenter__move-ordering__balanced` | `js/ai/learned-eval-profile.generated.js`, `js/ai/evaluation-profiles.js` |
| default move-ordering structure profile | `baseline-v1` | `js/ai/search-structure-profiles.js`, `js/ai/search-engine.js` |
| runtime tuple residual profile | `diagonal-top24-latea-endgame-patched-calibrated` | `js/ai/learned-eval-profile.generated.js`, `js/ai/evaluation-profiles.js` |
| runtime MPC profile | `stage154-main-recenter__runtime-mpc__overlapHighTight` | `js/ai/learned-eval-profile.generated.js`, `js/ai/evaluation-profiles.js` |
| default MPC structure profile | `baseline-v1` | `js/ai/search-structure-profiles.js`, `js/ai/search-engine.js` |
| stage157 mainline overlay support set | `s157-main-control`, `s157-main-wide-hybrid`, `s157-main-frontier-gate`, `s157-main-assertive-both`만 유지 | `tools/evaluator-training/stage157-structural-candidates.mjs`, `js/ai/search-structure-profiles.js` |
| stage158 mainline overlay registry | `s154-control`, `s154-stable-zebra`만 active registry에 남기고, `s154-stable-zebra`는 아직 installed default로 승격하지 않은 deferred survivor로만 유지 | `tools/evaluator-training/stage158-structural-candidates.mjs`, `tools/engine-match/prepare-stage157-158-mainline-decision-pack.mjs` |
| stage176 survivor branch registry | active key는 비워 두고, 여섯 branch를 모두 historical-only retired key로 유지 | `tools/evaluator-training/stage176-survivor-branch-candidates.mjs`, `docs/reports/review/review-stage-182-stage176-finals-failure-analysis-and-next-directions.md` |
| classic custom variant toggle | 기본 추천(stage154 main-recenter 설치본 유지), 사용자 지정에서 확장 후보형(stage154 both 번들 override) 선택 가능 | `js/ai/runtime-engine-variants.js`, `js/ai/presets.js`, `js/ui/settings-panel-view.js` |
| tooling historical alias | benchmark/tooling에 남아 있는 `active` variant key는 현재 설치 런타임 generated module을 가리키는 harness alias | `tools/engine-match/*`, `docs/reports/*` |
| exact micro-solver threshold | `optimizedFewEmptiesExactSolverEmpties = 8` | `js/ai/search-engine.js` |
| WLD micro-solver threshold | `optimizedFewEmptiesWldSolverEmpties = 8` | `js/ai/search-engine.js` |
| allocation-light search move path | 활성 (`allocationLightSearchMoves = true`, `reusablePreparedSearchMoveBuffers = true`, `lazyPreparedSearchMoves = true`, `tokenizedPreparedSearchMoveCore = true`, `compactPreparedSearchMoveFlips = false`) | `js/core/rules.js`, `js/ai/search-engine.js` |
| search-side hotpath follow-up experiments | experimental opt-in only (`ttFirstDeferredMoveListBuild = false`, `lowOverheadSearchChildStateFactory = false`) | `js/ai/search-engine.js` |
...
| refined few-empties WLD solver | 활성 (`optimizedFewEmptiesWldSolver = true`, `lightweightFewEmptiesWldMovePath = true`, `specializedFewEmptiesWldLastFlipPath = true`, `fewEmptiesWldFastestFirstSelectiveGate = true`) | `js/ai/search-engine.js` |
...
현재 기준 문서에서는 `active`라는 현재형 이름을 더 이상 런타임 프로필명으로 쓰지 않습니다. 다만 아래 historical stage 설명이나 benchmark/tooling 문맥에서 `active`가 보이면, 이는 당시 표현이거나 현재 설치 런타임 generated module을 뜻하는 harness alias로 읽으면 됩니다.

search skeleton의 structure profile layer는 위 installed generated module과 별도의 **선택형 overlay 계층**입니다. 현재 기본 key는 move-ordering/MPC 모두 `baseline-v1`로 유지하고, Stage 182 closeout 기준 stage157 mainline 채택분은 계속 runtime/tooling support set에 남겨 두었습니다. 따라서 `wide-hybrid-v1`, `verify-tight-v1`는 공식 structure library 항목으로 유지됩니다. 동시에 stage158 mainline에서는 `s154-stable-zebra`만 active registry에 남기고, `s154-anchor-main`, `s154-stable-quiet`, `s154-stable-quiet-probe`, `s154-stable-zebra-open`, `s154-zebra-both-probe`는 보고서 기록만 남긴 retired key로 정리했습니다. 여기에 Stage 181 결선 closeout 기준 stage176 survivor branch 6개도 전부 historical-only retired key로 내렸고, 어떤 stage170/stage176 mixed overlay도 installed default/runtime variant로 승격하지 않았습니다.
...
| `js/core/rules.js` | 합법 수 생성, 뒤집기 계산, prepared search move record 생성 | move generation hotpath + allocation-light prepared move path + eager/lazy prepared builder + reusable prepared buffer + tokenized prepared core + optional compact flip-token pilot + prefix-bidirectional/ray-between runtime defaults |
| `js/core/game-state.js` | 상태, 패스, 수 적용 | 탐색 노드 상태 컨테이너 |
| `js/ai/evaluation-profiles.js` | generated/seed profile compile과 runtime profile 노출 | evaluation / ordering / tuple / MPC 진입점 |
| `js/ai/pattern-bank.js` | pattern-bank layout/profile compile과 runtime scorer | `packed-lookup` scorer가 기본이며, legacy scorer는 비교용 fallback으로 유지 |
| `js/ai/search-structure-profiles.js` | optional move-ordering/MPC structure profile library와 normalize helper | Stage 182 closeout 기준 stage157 채택 overlay(`wide-hybrid-v1`, `verify-tight-v1`)와 기존 built-in skeleton 조합을 현재형 표면으로 유지 |
| `js/ai/evaluator.js` | phase-bucket evaluator, tuple residual, stability/feature 계산, move-ordering evaluator | Stage 86 stability hotpath flattening + Stage 190 packed pattern-bank scoring fast path |
| `js/ai/opening-book.js` | curated opening book 조회 | direct use + root ordering 참고 |
| `js/ai/opening-prior.js` | factorized compact opening prior 조회 | Stage 151 factorized binary-varint runtime export, book 보조와 ordering bias |
| `js/ai/opening-tuning.js` | opening hybrid key와 threshold/profile 해석 | 기본 key는 `stage59-cap9-prior-veto` |
| `js/ai/presets.js` | 난이도/스타일/custom 입력 해석 | 사용자 노출 설정 표면, classic runtime variant select 포함 |
| `js/ai/special-endings.js` | special-ending scout / trap penalty / immediate wipeout 공용 휴리스틱 | classic과 MCTS가 공유 |
| `js/ai/runtime-engine-variants.js` | stage154 classic runtime variant catalog/helper | 기본 추천(stage154 main-recenter 설치본)과 확장 후보형(stage154 both 번들) 선택을 metadata + profile override로 해석 |
| `js/ai/search-engine.js` | 탐색 엔진 본체 | opening → classic search / MCTS search → exact/WLD lane 통합, direct 기본 경로도 stage154 main runtime MPC profile 상속, classic 내부 노드는 reusable prepared buffer + selective lazy materialization + tokenized prepared core가 포함된 prepared search move path를 기본 사용하고, optional compact prepared flip-token path는 비교용으로 남기되 기본은 off이며, exact tail 5~8 empties는 lightweight reusable move path를 기본 사용합니다. Stage 203에서 search-side TT-first deferred move-list build와 low-overhead child-state factory도 다시 검증했지만 aggregate 채택선은 넘지 못해 둘 다 experimental opt-in으로만 남습니다. |
| `js/ai/worker.js` | 워커용 search wrapper | explicit `mpcProfile`가 없을 때 stage154 main runtime MPC semantics 유지 |
| `js/ui/engine-client.js` | 워커 우선 + main-thread fallback | UI와 direct engine의 stage154 main runtime MPC 기본 의미론을 동일하게 유지 |
...
Stage 195에서는 이 prepared move path를 현재 stage192 snapshot 코드베이스에 다시 forward-port해, `listPreparedSearchMoves()` 계열에 **eager/lazy prepared builder**, `createPreparedSearchMoveBuffer()` / `listPreparedSearchMovesIntoBuffer()` 기반 **per-ply reusable prepared buffer**, `materializePreparedSearchMoveFlipCount()` / `materializePreparedSearchMove()` 기반 **selective lazy flip materialization**을 추가했습니다. classic search에서는 같은 ply 재진입 위험이 없는 일반 recursive lane에서 reusable buffer를 사용하고, `shouldPrecomputeOrderingOutcome()`이 꺼진 non-precompute lane에서만 lazy path를 켜도록 연결했습니다. 초기 single-order / reverse-order benchmark는 tiny exact bucket에서 warm-order sensitivity를 보였기 때문에, 채택 판단은 per-state warm-up + interleaved balanced rerun을 기준으로 했습니다. balanced benchmark에서는 search-move micro `0.434x`, depth-limited 24 empties `1.004x`, depth-limited 20 empties `0.973x`, WLD-14 `0.983x`, exact-10 `0.988x`였고, best move / score / mode / nodes parity는 전 section에서 유지됐습니다. 즉 이 Stage는 **deeper midgame에서 소폭 이득, late bucket에서는 사실상 중립인 semantics-preserving hotpath cleanup**으로 채택하고 기본값을 유지합니다.

Stage 196에서는 이 prepared move path 위에 **tokenized prepared-search-move core**를 얹어, search hotpath의 prepared record에서 dedicated `bit` BigInt field를 제거하고 `flipCount`를 compact numeric token에 흡수했습니다. `bitFromIndex()`는 lookup table로 바꿔 bitless core의 보조 비용을 낮췄고, `SearchEngine`에는 `tokenizedPreparedSearchMoveCore = true`를 기본값으로 연결했습니다. 비교선은 `tokenizedPreparedSearchMoveCore: false`로 재현할 수 있습니다. balanced benchmark에서는 prepared move micro eager `0.983x`, lazy `1.044x`, depth-limited 24 empties `1.026x`, depth-limited 20 empties `0.996x`, WLD-14 `0.992x`, exact-10 `0.954x`였고, best move / score / mode / nodes parity는 전 section에서 유지됐습니다. 즉 이 Stage는 **대체로 near-neutral runtime cleanup이지만 compact-token 후속 작업으로 이어질 groundwork**로 채택하고 기본값을 유지합니다.

Stage 197에서는 exact-tail 쪽의 다음 확장 구간이었던 **5~8 empties lightweight few-empties exact move path**를 채택했습니다. `SearchEngine`은 empties 5~8 구간에서 fixed slot buffer를 재사용하는 lightweight exact-tail 경로를 기본으로 사용하고, `solveSmallExactBoards()`는 기존 object-heavy `generateFewEmptiesExactMoves()` 대신 그 경로로 직접 들어갈 수 있습니다. primary benchmark(legacy exact-tail threshold-6 baseline vs lightweight exact-tail + threshold-8 candidate)에서는 direct 5~8 empties micro가 `1.202x`로 느렸지만, exact-10 root는 `0.794x`, exact-12 root는 `0.981x`, WLD-14는 `0.988x`였고 best move / score / mode parity가 유지됐습니다. 이어서 threshold-only revalidation(lightweight path on, `6 -> 8`)을 따로 돌린 결과 exact-10 `0.845x`, exact-12 `0.995x`, WLD-14 `1.044x`였는데, WLD-14는 small-solver/path usage가 `0`이라 timing noise로 보는 편이 맞습니다. 그래서 현재 기본값은 `lightweightFewEmptiesExactMovePath = true`, `optimizedFewEmptiesExactSolverEmpties = 8`로 올렸고, 이 Stage는 **exact-root throughput / node reduction adoption**으로 기록합니다.

Stage 198에서는 Stage 196의 tokenized prepared core 뒤에 남아 있던 다음 후보였던 **compact prepared-search-move flip token + token-first apply path**를 구현해, prepared move token 안에 방향별 flip count를 눌러 담고 `GameState.applyMoveFast()`가 그 token을 그대로 먹을 수 있게 연결했습니다. 비교선은 기존 bigint flips 유지(`compactPreparedSearchMoveFlips: false`), 후보는 compact token path(`compactPreparedSearchMoveFlips: true`)로 잡았고, balanced split benchmark에서는 prepared move micro eager `1.099x`, lazy materialize/apply micro `1.221x`, depth-limited 24 empties `1.010x`, depth-limited 20 empties `1.007x`, WLD-14 `1.031x`, exact-10 `1.012x`였습니다. best move / score / mode / nodes parity는 전 section에서 유지됐지만 elapsed는 전반적으로 baseline보다 약간 느렸기 때문에, 이 Stage는 **semantics-preserving experimental path만 유지하고 기본값 채택은 보류**한 것으로 기록합니다. 즉 현재 런타임에서는 compact flip-token path가 남아 있더라도 기본값은 계속 `compactPreparedSearchMoveFlips = false`입니다.

Stage 199에서는 다음 유력 후보 묶음이었던 **few-empties WLD tail bundle**을 채택했습니다. `1~4 empties`에는 specialized WLD tail을, `5~8 empties`에는 lightweight reusable-slot WLD path를 붙였고, 현재 기본값은 `optimizedFewEmptiesWldSolver = true`, `optimizedFewEmptiesWldSolverEmpties = 8`, `lightweightFewEmptiesWldMovePath = true`입니다. direct `solveSmallWld()` micro만 떼어 보면 specialized 4-empty `1.978x`, direct 5~8 tail `1.261x`로 느렸지만, balanced benchmark 기준 실제 WLD search에서는 `wld10 0.613x`, `wld12 0.821x`, `wld14 0.781x`, search aggregate elapsed `0.784x`, search aggregate nodes `0.067x`로 크게 개선됐습니다. 따라서 이 Stage는 **direct micro 가속이 아니라 실제 WLD root throughput 개선 채택**으로 보는 편이 맞습니다.

Stage 200에서는 보고서에 남아 있던 exact-tail 후보였던 **specialized few-empties last-flip exact path**를 채택했습니다. 이 경로는 specialized exact 1~4 empties solver가 마지막 수 근처에서 `bitsToIndices()` 재생성·말 수 재count 대신 index 기반 flip query(`computeFlipCountAtIndex()` / `computeFlipsAtIndex()`)와 disk-diff carry를 직접 이어 받아 마무리하도록 바꿉니다. 현재 기본값은 `specializedFewEmptiesLastFlipPath = true`입니다. primary benchmark에서는 direct 1-empty micro `0.507x`, exact-10 root `0.957x`, exact-12 root `0.944x`, WLD-12 control `0.995x`가 나왔고, 후속 balanced/current rerun에서도 exact aggregate elapsed ratio가 각각 `0.910x`, `0.912x`로 유지됐습니다. best move / score / mode / nodes parity는 전 run에서 유지됐고 WLD control은 last-flip telemetry가 `0`이었습니다. 즉 이 Stage는 **few-empties exact tail의 low-risk speedup adoption**으로 기록하는 것이 맞습니다.

Stage 201에서는 다음 잔여 exact-tail 후보였던 **few-empties exact fastest-first selective gate**를 채택했습니다. 기존 exact fastest-first ordering은 reply-count 기반이라 low-branching few-empties node에서도 상대 legal count probe와 정렬 비용을 그대로 치렀는데, 이번 Stage에서는 few-empties exact helper path 안에서 legal move 수가 `4` 미만이면 reply-count fastest-first를 건너뛰고 기존 square-score ordering만 사용하도록 좁게 제한했습니다. 현재 기본값은 `fewEmptiesExactFastestFirstSelectiveGate = true`입니다. direct 8-empty micro 자체는 `1.193x`, rerun `1.103x`로 느렸지만, exact-12 root는 `0.961x`, rerun `0.965x`, exact search aggregate는 `0.970x`, rerun `0.973x`로 일관되게 개선됐고 exact-10은 사실상 중립이었습니다. WLD-12 control은 `0.996x`와 `1.019x`로 거의 중립이었고, best move / score / mode parity는 전 section에서 유지됐습니다. exact-12 한 seed에서는 nodes가 `221 -> 229`로 소폭 늘어 aggregate node ratio가 `1.007x`가 됐지만, elapsed 이득 쪽이 두 run 모두 반복돼 이번 Stage는 **direct micro 가속이 아니라 exact-root throughput 개선 채택**으로 기록합니다.

Stage 202에서는 그 다음 WLD-tail 정리 후보였던 **few-empties WLD tail refinement bundle**을 채택했습니다. 이번 묶음은 Stage 199의 WLD 1~8 tail 위에 남아 있던 두 후속 정리, 즉 **specialized few-empties WLD last-flip path**와 **few-empties WLD fastest-first selective gate**를 함께 기본 경로에 연결한 것입니다. 현재 기본값은 `specializedFewEmptiesWldLastFlipPath = true`, `fewEmptiesWldFastestFirstSelectiveGate = true`입니다. main benchmark에서는 direct 1-empty WLD micro가 `0.912x`, direct 8-empty WLD micro가 `0.580x`였고, WLD root는 `wld10 0.904x`, `wld12 0.894x`, `wld14 0.885x`, search aggregate는 `0.888x`였습니다. rerun에서는 direct 1-empty가 `1.005x`로 거의 중립이었지만 direct 8-empty는 `0.587x`, WLD root는 `wld10 1.019x`, `wld12 0.864x`, `wld14 0.901x`, search aggregate는 `0.896x`로 다시 개선됐습니다. best move / score / mode / nodes parity는 전 section에서 유지됐고 exact-12 control도 `0.974x`, rerun `0.966x`로 neutral-to-better였습니다. aggregate search node ratio는 `1.000x`로 그대로였고 WLD small-solver node ratio만 `1.021x`로 소폭 흔들렸으므로, 이번 이득은 node reduction보다 **few-empties WLD tail 내부의 ordering / leaf overhead cleanup**에서 왔다고 읽는 편이 맞습니다. `wld10` rerun wobble은 짧은 four-way isolate에서 `lastFlipOnly 0.742x`, `gateOnly 0.749x`, `both 0.652x`로 다시 확인해 원인이 bundle 전체의 구조적 퇴행이 아님을 점검했습니다. 따라서 이번 Stage는 **WLD-root throughput 개선 채택**으로 기록합니다.

Stage 203에서는 다음 search-side hotpath 후보 묶음인 **TT-first deferred move-list build**와 companion **low-overhead child-state factory**를 split balanced benchmark로 다시 검증했습니다. monolithic runner는 현재 실행 환경에서 transposition-table/process pressure 때문에 안정적으로 끝나지 않아, 이번에는 sample마다 fresh Node process를 띄우는 split harness를 사용했습니다. 결과는 채택 쪽이 아니었습니다. deferred-only는 depth-limited 24 empties가 `1.044x`, rerun `1.009x`, depth-limited 20 empties가 `1.001x`, rerun `1.016x`, WLD-14 control이 `1.007x`, exact-10 control이 `0.996x`였고, best move / score / mode / nodes parity는 모두 유지됐습니다. 즉 semantics는 보존됐지만 aggregate throughput 이득은 확인되지 않았습니다. low-overhead child-state factory는 depth-limited 20 empties에서 `0.965x`로 개선됐지만, depth-limited 24 empties `1.008x`, WLD-14 control `1.052x`, exact-10 control `1.042x`로 다른 bucket에서 되돌림이 나타났습니다. 따라서 Stage 203은 **search hotpath revalidation no-adoption**으로 기록하고, `ttFirstDeferredMoveListBuild = false`, `lowOverheadSearchChildStateFactory = false`를 기본값으로 유지합니다.

Stage 204는 runtime semantics를 더 바꾸는 단계가 아니라, Stage 192의 remainder map을 phase2 결과까지 갱신하고, search/tail 기본값 authority를 exported helper(`DEFAULT_SEARCH_MOVE_PATH_RUNTIME_OPTIONS`, `DEFAULT_FEW_EMPTIES_EXACT_RUNTIME_OPTIONS`, `DEFAULT_FEW_EMPTIES_WLD_RUNTIME_OPTIONS`, `getSearchRuntimeDefaultConfig()`)로 고정한 **closeout/refactor stage**였습니다. exact/WLD few-empties lightweight path의 공통 move-buffer build도 `populateFewEmptiesMoveBuffer()` helper로 합쳐, Stage 197/199/201/202에서 누적된 tail duplication을 runtime behavior 변경 없이 정리했습니다.
...
- allocation-light prepared search move record path + reusable prepared buffer + selective lazy flip materialization
- optional compact prepared flip-token path (`compactPreparedSearchMoveFlips = false` 기본 유지)
- runtime MPC profile을 이용한 conservative fail-high 중심 runtime lane
...
- `5~8` empties: lightweight reusable move path가 붙은 optimized exact micro-solver tail window
- exact lane 내부에서는 exact fastest-first reply-count ordering을 유지하되, few-empties exact helper path에서는 legal move 수가 `4` 미만이면 selective gate로 reply-count probe를 건너뜁니다

`optimizedFewEmptiesExactSolverEmpties`의 현재 기본 threshold는 `8`입니다.
Stage 197 revalidation에서는 lightweight exact-tail path를 함께 둔 상태에서 threshold `6 -> 8` 확장을 다시 검증했고, exact 10 empties는 `0.845x`, exact 12 empties는 `0.995x`로 확인되어 기본값을 넓혀 채택했습니다. direct 5~8 empties only micro는 cold-start 기준 `1.202x`로 느렸기 때문에, 이번 채택은 direct micro win이라기보다 **exact-root throughput 개선과 node 절감 쪽 채택**으로 읽는 편이 맞습니다.
...
- custom 수치 입력
  - classic 계열: 깊이, aspiration window, 클래식 읽기 성향(기본 추천 / 확장 후보형)
...
### 내부에서 고정된 기본 런타임 선택
- active evaluation / move-ordering / tuple residual / MPC profile 이름과 기본 classic runtime variant(stage154 main)
...
- exact fastest-first ordering on (`fewEmptiesExactFastestFirstSelectiveGate = true`)
- worker / UI fallback / direct `SearchEngine`의 runtime MPC 기본 의미론 정렬
...
- Stage 124~147 compact tuple lane은 evaluation / tuple residual 기반을 현재 코드에도 남겼지만, 현재 설치 기본 generated module은 stage167 benchmark reset 후속 정리에 따라 **stage154 main-recenter runtime**으로 교체되었습니다. 그래서 runtime tuple residual은 계속 `diagonal-top24-latea-endgame-patched-calibrated`이지만, runtime move-ordering과 MPC는 각각 `stage154-main-recenter__move-ordering__balanced`, `stage154-main-recenter__runtime-mpc__overlapHighTight`를 사용합니다. 이전 compact-tuple 설치본은 `tools/engine-match/fixtures/historical-installed-modules/active-precompact-tuple.learned-eval-profile.generated.js`에 archive되어 있습니다. Stage 170/176 mixed-overlay survivor lane도 최종적으로 baseline을 넘지 못했기 때문에, 현재 기본 runtime에는 남기지 않습니다.
- 사용자 지정 난이도에서는 `클래식 읽기 성향`으로 기본 추천(stage154 main)과 확장 후보형(stage154 both)을 고를 수 있습니다. 확장 후보형은 후보폭을 조금 더 넓히고 양방향 컷까지 함께 쓰지만, 현재 기본값은 여전히 기본 추천입니다. overlay 계열은 지금 단계에서는 설치 기본값에 올리지 않았습니다.
