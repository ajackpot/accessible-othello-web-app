...
function resolveVariantSpec(rawSpec, index) {
  ...
  return {
    ...
    engineOptionsJson: resolvePathMaybe(rawSpec.engineOptionsJson),
...
function resolveScenarioSpec(rawSpec, defaults, index) {
  ...
  return {
    ...
    searchAlgorithm: typeof merged.searchAlgorithm === 'string' && merged.searchAlgorithm.trim() !== ''
      ? merged.searchAlgorithm.trim()
      : String(defaults.searchAlgorithm ?? 'classic'),
    aspirationWindow: Number(merged.aspirationWindow ?? defaults.aspirationWindow ?? 40),
    maxTableEntries: Number(merged.maxTableEntries ?? defaults.maxTableEntries ?? 180000),
    raw: cloneJsonValue(rawSpec),
  };
}
...
function buildBenchmarkCommand(variant, scenario, outputJsonPath) {
  const args = [
    ...
    '--search-algorithm', String(scenario.searchAlgorithm),
  ...
  if (variant.type === 'custom') {
    ...
    if (variant.engineOptionsJson) {
      args.push('--engine-options-json', variant.engineOptionsJson);
    }
...
function normalizeRunSummary(runOutput, entry) {
  ...
  return {
    ...
    scenarioSearchAlgorithm: entry.scenario.searchAlgorithm,
    scenarioAspirationWindow: entry.scenario.aspirationWindow,
    scenarioMaxTableEntries: entry.scenario.maxTableEntries,
...
console.log(`Scenarios           : ${scenarios.map((entry) => `${entry.id}(games=${entry.games}, seed=${entry.seed}, algo=${entry.searchAlgorithm})`).join(', ')}`);
