...
const DEFAULT_OUTPUT_ROOT = path.resolve(PROJECT_ROOT, 'tools', 'engine-match', 'out', 'stage170-survivor-decision-runs');
...
function printUsage() {
  ...
  node tools/engine-match/run-stage170-survivor-decision-pair.mjs \
    --candidate <stage170-combo-or-stage157-158-survivor-key> \
...
설명:
- 남은 stage157/stage158 survivor 또는 새 stage170 combo 후보를 's154-main', 's154-both'에 각각 overlay한 뒤,
  direct pair benchmark를 80/160/240ms 같은 다중 시간 구간에서 순차 실행합니다.
...
function parseBaselineList(value) {
  if (typeof value !== 'string' || value.trim() === '') {
    return BASELINES;
  }
  const ids = [...new Set(value.split(',').map((token) => token.trim()).filter(Boolean))];
  const selected = BASELINES.filter((baseline) => ids.includes(baseline.id));
  if (selected.length === 0) {
    throw new Error(`Unknown baseline selection: ${value}`);
  }
  return selected;
}
...
function writeMarkdownSummary(filePath, summary) {
  const lines = [];
  lines.push(`# Survivor decision pair summary for ${summary.candidate.key}`);
  lines.push('');
...
const selectedBaselines = parseBaselineList(args.baseline ?? args['baseline-list'] ?? '');
...
const finalSummary = {
  ...
  options: {
    ...
    aspirationWindow,
    progressEveryPairs,
    baselineIds: selectedBaselines.map((baseline) => baseline.id),
  },
  runs: runRecords,
  rows: summaryRows,
};
