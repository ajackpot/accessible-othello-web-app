...
이 문서에서 built-in variant key `active`는 **현재 설치 런타임 generated module**을 가리키는 benchmark harness alias입니다. 현재 `js/ai` 런타임 프로필의 공식 명칭이 아니라는 점만 구분해서 읽으면 됩니다.
...
- **학습 전/후 비교**: 기본값으로 현재 설치 generated module(benchmark harness built-in key는 `active`, 현재 설치본은 stage154 main-recenter runtime), learned evaluator만 적용한 비교군(`phase-only`), 학습 전 seed evaluator(`legacy`)를 함께 돌려 비교
...
- `benchmark-profile-variant-throughput-compare.mjs`
  - installed-runtime(`active` alias) / finalist generated module을 같은 root position 묶음에서 비교해 profile variant별 처리량, 완료 깊이, move agreement를 계산합니다.
...
- `run-stage147-runtime-install-and-post-adoption-validation.mjs` / `run-stage147-runtime-install-and-post-adoption-validation.bat`
  - Stage 146에서 선택한 final generated module을 실제 installed runtime에 설치하고, 설치 직전 module은 historical fixture로 archive합니다.
  - archived previous-installed 대비 paired self-play / throughput / explicit search-cost(depth/exact) post-adoption validation을 다시 수행하고, parity 실패나 regression이 있으면 자동으로 previous-installed로 rollback합니다.
...
이 구성은 현재 설치 런타임(`active` alias)과, 같은 generated module에서 `--disable-mpc` 만 적용한 비교군을
...
- `active`는 benchmark harness alias이며, 현재 `learned-eval-profile.generated.js`에 들어 있는 **설치 런타임 generated evaluator / move-ordering / tuple / MPC 조합**을 그대로 사용합니다.
