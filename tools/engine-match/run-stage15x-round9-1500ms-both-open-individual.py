...
OUT_DIR = REPO / 'tools' / 'engine-match' / 'out' / '_stage15x_restart_round9_1500ms_both_open_individual'
...
def run_one(variant, scenario):
    ...
    out_json_rel = f"tools/engine-match/out/_stage15x_restart_round9_1500ms_both_open_individual/results/{run_id}.json"
...
def main():
    ...
    aggregate_json = OUT_DIR / 'stage15x_restart_round9_1500ms_both_open_aggregate.json'
    aggregate_csv = OUT_DIR / 'stage15x_restart_round9_1500ms_both_open_aggregate.csv'
    summary_md = OUT_DIR / 'stage15x_restart_round9_1500ms_both_open_summary.md'
    manifest_json = OUT_DIR / 'stage15x_restart_round9_1500ms_both_open_manifest.json'
    ...
    lines = []
    lines.append('# Stage15x restart round 9: 1500ms noisy with s154-both + s158-stable-zebra-open')
    lines.append('')
    ...
    lines.append('- Variants: `active`, `s154-main + s158-stable-zebra-open`, `s154-both + s158-stable-zebra-open`, `s154-both`, `s154-main`.')
    lines.append('')
    ...
    lines.append('- This round is still noisy-only, but the sample is tripled relative to the 3000ms check and stays in the same individual-run aggregation format.')
    lines.append('- The key read should therefore be whether the stage154 base/overlay ordering stays stable when the budget drops from 3000ms to 1500ms.')
    lines.append('')
