...
function printUsage() {
  ...
  node ${toolPath} \
    ...
    [--shared-search-pairs auto|on|off] \
    [--shared-search-pairs-output-jsonl tools/evaluator-training/out/shared-mpc-search-pairs.jsonl] \
...
- sharedSearchPairs.mode=auto|on 이면 후보 전체 bucket union에 대해 shallow/deep search pair를 **한 번만** precompute 하고, 각 후보는 그 캐시에서 회귀만 다시 피팅합니다.
...
async function materializeActiveProfileSnapshots(sharedDir) {
  ...
  const moveOrderingPath = RUNTIME_MOVE_ORDERING_PROFILE ? path.join(sharedDir, 'active-move-ordering-profile.snapshot.json') : null;
  const mpcPath = RUNTIME_MPC_PROFILE ? path.join(sharedDir, 'active-mpc-profile.snapshot.json') : null;
  const tuplePath = RUNTIME_TUPLE_RESIDUAL_PROFILE ? path.join(sharedDir, 'active-tuple-residual-profile.snapshot.json') : null;

  await ensureJsonFile(evaluationPath, RUNTIME_EVALUATION_PROFILE ?? null);
  if (moveOrderingPath) {
    await ensureJsonFile(moveOrderingPath, RUNTIME_MOVE_ORDERING_PROFILE);
  }
  if (mpcPath) {
    await ensureJsonFile(mpcPath, RUNTIME_MPC_PROFILE);
  }
  if (tuplePath) {
    await ensureJsonFile(tuplePath, RUNTIME_TUPLE_RESIDUAL_PROFILE);
  }
...
const sharedSearchPairsConfig = isPlainObject(mergedConfig.sharedSearchPairs) ? mergedConfig.sharedSearchPairs : DEFAULT_CONFIG.sharedSearchPairs;
const sharedSearchPairsMode = typeof args['shared-search-pairs'] === 'string' && args['shared-search-pairs'].trim() !== ''
  ? args['shared-search-pairs'].trim().toLowerCase()
  : String(sharedSearchPairsConfig.mode ?? 'auto').trim().toLowerCase();
const candidateSampleStrides = new Set(normalizedCandidates.map((candidate) => `${candidate.sampleStride}|${candidate.sampleResidue}`));
const canShareSearchPairs = candidateSampleStrides.size <= 1;
const sharedSearchPairsEnabled = (sharedSearchPairsMode === 'on')
  || (sharedSearchPairsMode !== 'off' && normalizedCandidates.length > 1 && canShareSearchPairs);
const sharedSearchPairBucketSet = [...new Set(normalizedCandidates.flatMap((candidate) => candidate.calibrationBuckets))];
const sharedSearchPairsPlan = {
  mode: sharedSearchPairsMode,
  enabled: sharedSearchPairsEnabled,
  canShareSearchPairs,
  reason: canShareSearchPairs ? null : 'candidate sampleStride/sampleResidue differ',
  calibrationBuckets: sharedSearchPairBucketSet,
  maxSamplesPerBucket: normalizedCandidates.reduce((max, candidate) => Math.max(max, candidate.maxSamplesPerBucket), 0),
  sampleStride: normalizedCandidates[0]?.sampleStride ?? candidateDefaults.sampleStride,
  sampleResidue: normalizedCandidates[0]?.sampleResidue ?? candidateDefaults.sampleResidue,
  timeLimitMs: normalizedCandidates.reduce((max, candidate) => Math.max(max, candidate.timeLimitMs), 0),
  progressEvery: normalizedCandidates.reduce((max, candidate) => Math.max(max, candidate.progressEvery), 0),
  maxTableEntries: normalizedCandidates.reduce((max, candidate) => Math.max(max, candidate.maxTableEntries), 0),
  aspirationWindow: normalizedCandidates.reduce((max, candidate) => Math.max(max, candidate.aspirationWindow), 0),
  outputJsonlPath: resolveCliOrConfigPath(
    args['shared-search-pairs-output-jsonl'],
    sharedSearchPairsConfig.outputJsonl,
    configBaseDir,
  ) ?? path.join(sharedDir, 'shared-mpc-search-pairs.jsonl'),
  summaryJsonPath: resolveCliOrConfigPath(
    args['shared-search-pairs-summary-json'],
    sharedSearchPairsConfig.summaryJson,
    configBaseDir,
  ) ?? path.join(sharedDir, 'shared-mpc-search-pairs.summary.json'),
  checkpointJsonPath: resolveCliOrConfigPath(
    args['shared-search-pairs-checkpoint-json'],
    sharedSearchPairsConfig.checkpointJson,
    configBaseDir,
  ) ?? path.join(sharedDir, 'shared-mpc-search-pairs.checkpoint.json'),
  checkpointEvery: toFiniteInteger(sharedSearchPairsConfig.checkpointEvery, 50),
};

const toolPaths = {
  calibrateMpc: resolveTrainingToolPath('calibrate-mpc-profile.mjs'),
  precomputeMpcPairs: resolveTrainingToolPath('precompute-mpc-search-pairs.mjs'),
  fitMpcFromPairs: resolveTrainingToolPath('fit-mpc-profile-from-search-pairs.mjs'),
...
const resolvedRunConfig = {
  ...
  sharedSearchPairs: {
    mode: sharedSearchPairsPlan.mode,
    enabled: sharedSearchPairsPlan.enabled,
    canShareSearchPairs: sharedSearchPairsPlan.canShareSearchPairs,
    reason: sharedSearchPairsPlan.reason,
    calibrationBuckets: sharedSearchPairsPlan.calibrationBuckets,
    maxSamplesPerBucket: sharedSearchPairsPlan.maxSamplesPerBucket,
    sampleStride: sharedSearchPairsPlan.sampleStride,
    sampleResidue: sharedSearchPairsPlan.sampleResidue,
    timeLimitMs: sharedSearchPairsPlan.timeLimitMs,
    progressEvery: sharedSearchPairsPlan.progressEvery,
    maxTableEntries: sharedSearchPairsPlan.maxTableEntries,
    aspirationWindow: sharedSearchPairsPlan.aspirationWindow,
    outputJsonlPath: relativeToCwd(sharedSearchPairsPlan.outputJsonlPath),
    summaryJsonPath: relativeToCwd(sharedSearchPairsPlan.summaryJsonPath),
    checkpointJsonPath: relativeToCwd(sharedSearchPairsPlan.checkpointJsonPath),
    checkpointEvery: sharedSearchPairsPlan.checkpointEvery,
  },
...
const suiteStatusPath = path.join(outputDir, 'suite-status.json');
const suiteStatus = loadStatusFile(suiteStatusPath, {
  scope: 'suite',
  createdAt: new Date().toISOString(),
  steps: {},
});

if (sharedSearchPairsPlan.enabled && sharedSearchPairsPlan.calibrationBuckets.length > 0) {
  const precomputeArgs = [];
  pushMultiArg(precomputeArgs, 'input', inputPaths);
  pushArg(precomputeArgs, 'evaluation-profile-json', sharedEvaluationProfilePath);
  pushArg(precomputeArgs, 'move-ordering-profile-json', sharedMoveOrderingProfilePath);
  pushArg(precomputeArgs, 'tuple-profile-json', sharedTupleProfilePath);
  pushArg(precomputeArgs, 'calibration-buckets', sharedSearchPairsPlan.calibrationBuckets.join(','));
  pushArg(precomputeArgs, 'sample-stride', sharedSearchPairsPlan.sampleStride);
  pushArg(precomputeArgs, 'sample-residue', sharedSearchPairsPlan.sampleResidue);
  pushArg(precomputeArgs, 'max-samples-per-bucket', sharedSearchPairsPlan.maxSamplesPerBucket);
  pushArg(precomputeArgs, 'time-limit-ms', sharedSearchPairsPlan.timeLimitMs);
  pushArg(precomputeArgs, 'progress-every', sharedSearchPairsPlan.progressEvery);
  pushArg(precomputeArgs, 'max-table-entries', sharedSearchPairsPlan.maxTableEntries);
  pushArg(precomputeArgs, 'aspiration-window', sharedSearchPairsPlan.aspirationWindow);
  pushArg(precomputeArgs, 'output-jsonl', sharedSearchPairsPlan.outputJsonlPath);
  pushArg(precomputeArgs, 'summary-json', sharedSearchPairsPlan.summaryJsonPath);
  pushArg(precomputeArgs, 'checkpoint-json', sharedSearchPairsPlan.checkpointJsonPath);
  pushArg(precomputeArgs, 'checkpoint-every', sharedSearchPairsPlan.checkpointEvery);
  pushFlag(precomputeArgs, 'resume', resume);

  const precomputeResult = await runTrackedStep({
    status: suiteStatus,
    statusPath: suiteStatusPath,
    stepKey: 'shared-search-pairs',
    description: 'shared MPC search-pair precompute',
    toolPath: toolPaths.precomputeMpcPairs,
    toolArgs: precomputeArgs,
    outputs: [sharedSearchPairsPlan.outputJsonlPath, sharedSearchPairsPlan.summaryJsonPath],
    dryRun,
    resume,
    cwd: process.cwd(),
  });
  suiteSummary.sharedSearchPairs.stepStatus = precomputeResult.status;
}
...
for (const candidate of normalizedCandidates) {
  ...
  try {
    const calibrateArgs = [];
    let calibrateToolPath = toolPaths.calibrateMpc;
    let calibrateStepKey = 'calibrate-mpc-profile';
    let calibrateDescription = `${candidate.key}: calibrate MPC`;
    if (sharedSearchPairsPlan.enabled) {
      pushArg(calibrateArgs, 'search-pairs-jsonl', sharedSearchPairsPlan.outputJsonlPath);
      calibrateToolPath = toolPaths.fitMpcFromPairs;
      calibrateStepKey = 'fit-mpc-profile-from-search-pairs';
      calibrateDescription = `${candidate.key}: fit MPC from shared pairs`;
    } else {
    ...
    const calibrateResult = await runTrackedStep({
      ...
      stepKey: calibrateStepKey,
      description: calibrateDescription,
      toolPath: calibrateToolPath,
