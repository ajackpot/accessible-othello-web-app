...
const candidateProfile = loadJsonFileIfPresent(args['move-ordering-profile']) ?? RUNTIME_MOVE_ORDERING_PROFILE;
if (!candidateProfile) {
...
const teacherEvaluationProfile = loadJsonFileIfPresent(args['teacher-evaluation-profile']) ?? RUNTIME_EVALUATION_PROFILE;
const teacherMoveOrderingProfile = loadJsonFileIfPresent(args['teacher-move-ordering-profile']) ?? RUNTIME_MOVE_ORDERING_PROFILE ?? null;
