...
const DEFAULT_OUTPUT_ROOT = resolveProjectPath('tools', 'evaluator-training', 'out', 'stage157-structural-smoke');
...
function printUsage() {
  console.log(`Usage:
  node tools/evaluator-training/run-stage157-structural-candidate-smoke.mjs \
    [--family stage154-main-recenter] [--family stage151-split-late3] \
    [--candidate s157-main-wide-hybrid] [--candidate s157-late3-anchor-noend] \
    ...
    [--output-root ${toPortablePath(DEFAULT_OUTPUT_ROOT)}] [--skip-aggressive]
...
function writeMarkdownSummary(summary) {
  const lines = [];
  lines.push('# Stage157 structural candidate smoke summary');
  lines.push('');
...
async function main() {
  ...
  if (candidateFilters.length > 0) {
    candidates = candidateFilters.map((key) => resolveStage157StructuralCandidate(key));
  } else if (familyFilters.length > 0) {
    candidates = familyFilters.flatMap((familyKey) => listStage157StructuralCandidates({ familyKey, includeAggressive: !skipAggressive }));
  } else {
    candidates = listStage157StructuralCandidates({ includeAggressive: !skipAggressive });
  }
  ...
  const summary = Object.freeze({
    ...
    purpose: 'stage157 structural move-ordering + MPC smoke',
  ...
  const summaryJsonPath = path.join(outputRoot, 'stage157_structural_smoke_summary.json');
  const summaryMdPath = path.join(outputRoot, 'stage157_structural_smoke_summary.md');
