...
function printUsage() {
  ...
  node ${toolPath} \
    ...
    [--teacher-generated-module path/to/learned-eval-profile.generated.js] \
    ...
    [--teacher-pattern-bank-json path/to/pattern-bank.json ...|off] \
    [--teacher-move-ordering-pattern-bank-json path/to/move-ordering-pattern-bank.json ...|off] \
    ...
    [--module-generated-module path/to/learned-eval-profile.generated.js] \
    ...
    [--pattern-bank-json path/to/pattern-bank.json ...|off] \
    [--move-ordering-pattern-bank-json path/to/move-ordering-pattern-bank.json ...|off]
...
const teacherGeneratedModule = await loadGeneratedProfilesModuleIfPresent(args['teacher-generated-module']);
const moduleGeneratedModule = await loadGeneratedProfilesModuleIfPresent(args['module-generated-module']);
...
const teacherTupleResidualProfileInput = loadJsonFileOrExplicitNull(args['teacher-tuple-profile-json'], teacherGeneratedModule?.tupleResidualProfile ?? RUNTIME_TUPLE_RESIDUAL_PROFILE ?? null);
const teacherMpcProfileInput = loadJsonFileOrExplicitNull(args['teacher-mpc-profile-json'], teacherGeneratedModule?.mpcProfile ?? RUNTIME_MPC_PROFILE ?? null);
const teacherPatternBankProfilesInput = loadJsonStackOrExplicitNull(args['teacher-pattern-bank-json'], teacherGeneratedModule?.patternBankProfiles ?? null);
const teacherMoveOrderingPatternBankProfilesInput = loadJsonStackOrExplicitNull(args['teacher-move-ordering-pattern-bank-json'], teacherGeneratedModule?.moveOrderingPatternBankProfiles ?? null);
...
const teacherEvaluationProfile = teacherEvaluationProfileInput ?? teacherGeneratedModule?.evaluationProfile ?? RUNTIME_EVALUATION_PROFILE;
const teacherMoveOrderingProfile = teacherMoveOrderingProfileInput ?? teacherGeneratedModule?.moveOrderingProfile ?? RUNTIME_MOVE_ORDERING_PROFILE ?? null;
const teacherTupleResidualProfile = teacherTupleResidualProfileInput;
const teacherMpcProfile = teacherMpcProfileInput;
const teacherPatternBankProfiles = teacherPatternBankProfilesInput;
const teacherMoveOrderingPatternBankProfiles = teacherMoveOrderingPatternBankProfilesInput;
const moduleEvaluationProfile = moduleEvaluationProfileInput ?? moduleGeneratedModule?.evaluationProfile ?? teacherEvaluationProfile ?? RUNTIME_EVALUATION_PROFILE;
const moduleTupleResidualProfile = loadJsonFileOrExplicitNull(args['tuple-profile-json'], moduleGeneratedModule?.tupleResidualProfile ?? teacherTupleResidualProfile);
const moduleMpcProfile = loadJsonFileOrExplicitNull(args['mpc-json'], moduleGeneratedModule?.mpcProfile ?? teacherMpcProfile);
const modulePatternBankProfiles = loadJsonStackOrExplicitNull(args['pattern-bank-json'], moduleGeneratedModule?.patternBankProfiles ?? teacherPatternBankProfiles);
const moduleMoveOrderingPatternBankProfiles = loadJsonStackOrExplicitNull(args['move-ordering-pattern-bank-json'], moduleGeneratedModule?.moveOrderingPatternBankProfiles ?? teacherMoveOrderingPatternBankProfiles);
...
console.log(`teacher pattern stack : eval=${teacherPatternBankProfiles?.length ?? 0} | ordering=${teacherMoveOrderingPatternBankProfiles?.length ?? 0}`);
...
try {
  await streamTrainingSamples(inputFiles, { targetScale: 3000 }, ({ state, sampleIndex }) => {
    ...
    const teacherOptions = createTeacherSearchOptions({
      ...
      patternBankProfiles: teacherPatternBankProfiles,
      moveOrderingPatternBankProfiles: teacherMoveOrderingPatternBankProfiles,
    });
...
if (outputModulePath) {
  savedModulePath = await writeGeneratedProfilesModule(outputModulePath, {
    ...
    patternBankProfiles: modulePatternBankProfiles,
    moveOrderingPatternBankProfiles: moduleMoveOrderingPatternBankProfiles,
  });
}
...
if (savedModulePath) {
  ...
  console.log(`  preserved pattern-bank stack  : eval=${modulePatternBankProfiles?.length ?? 0} | ordering=${moduleMoveOrderingPatternBankProfiles?.length ?? 0}`);
}
