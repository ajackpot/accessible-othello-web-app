...
function deriveDefaultConfigFromActiveProfiles() {
  return {
    tuple: {
      layoutName: RUNTIME_TUPLE_RESIDUAL_PROFILE?.layout?.name ?? 'orthogonal-adjacent-pairs-outer2-v1-patched-patched',
      phaseBuckets: Array.isArray(RUNTIME_TUPLE_RESIDUAL_PROFILE?.trainedBuckets)
        ? RUNTIME_TUPLE_RESIDUAL_PROFILE.trainedBuckets.map((bucket) => bucket.key ?? `${bucket.minEmpties}-${bucket.maxEmpties}`)
        : ['late-b', 'endgame'],
      sampleStride: 4,
      sampleResidue: 0,
      epochs: 1,
    ...
    moveOrdering: {
      childBuckets: Array.isArray(RUNTIME_MOVE_ORDERING_PROFILE?.trainedBuckets)
        ? RUNTIME_MOVE_ORDERING_PROFILE.trainedBuckets.map((bucket) => `${bucket.minEmpties}-${bucket.maxEmpties}`)
        : ['11-12', '15-16', '17-18'],
      sampleStride: 200,
      sampleResidue: 0,
    ...
    mpc: {
      calibrationBuckets: Array.isArray(RUNTIME_MPC_PROFILE?.calibrations)
        ? RUNTIME_MPC_PROFILE.calibrations.map((entry) => `${entry.minEmpties}-${entry.maxEmpties}:${entry.shallowDepth}>${entry.deepDepth}`)
    ...
    runtime: {
      defaultMode: (RUNTIME_MPC_PROFILE?.runtime?.enableLowCut === true)
        ? 'both'
        : ((RUNTIME_MPC_PROFILE?.runtime?.enableHighCut === false) ? 'off' : 'high'),
      enableHighCut: RUNTIME_MPC_PROFILE?.runtime?.enableHighCut !== false,
      enableLowCut: RUNTIME_MPC_PROFILE?.runtime?.enableLowCut === true,
      maxWindow: RUNTIME_MPC_PROFILE?.runtime?.maxWindow ?? 1,
      maxChecksPerNode: RUNTIME_MPC_PROFILE?.runtime?.maxChecksPerNode ?? 2,
      minDepth: RUNTIME_MPC_PROFILE?.runtime?.minDepth ?? 2,
      minDepthGap: RUNTIME_MPC_PROFILE?.runtime?.minDepthGap ?? 2,
      maxDepthDistance: RUNTIME_MPC_PROFILE?.runtime?.maxDepthDistance ?? 1,
      minPly: RUNTIME_MPC_PROFILE?.runtime?.minPly ?? 1,
      intervalScale: 1,
      highScale: RUNTIME_MPC_PROFILE?.runtime?.highScale ?? 1,
      lowScale: RUNTIME_MPC_PROFILE?.runtime?.lowScale ?? 1,
      depthDistanceScale: RUNTIME_MPC_PROFILE?.runtime?.depthDistanceScale ?? 1.25,
    },
...
async function prepareSharedInputs({
  ...
  if (RUNTIME_MOVE_ORDERING_PROFILE) {
    await ensureJsonFile(activeMoveOrderingPath, RUNTIME_MOVE_ORDERING_PROFILE);
  }
  if (RUNTIME_TUPLE_RESIDUAL_PROFILE) {
    await ensureJsonFile(activeTuplePath, RUNTIME_TUPLE_RESIDUAL_PROFILE);
  }
  if (RUNTIME_MPC_PROFILE) {
    await ensureJsonFile(activeMpcPath, RUNTIME_MPC_PROFILE);
  }
  ...
  return {
    ...
    activeMoveOrderingPath: RUNTIME_MOVE_ORDERING_PROFILE ? activeMoveOrderingPath : null,
    activeTuplePath: RUNTIME_TUPLE_RESIDUAL_PROFILE ? activeTuplePath : null,
    activeMpcPath: RUNTIME_MPC_PROFILE ? activeMpcPath : null,
    resolvedConfigPath,
  };
}
...
const sharedPaths = args.planOnly
  ? {
      ...
      activeMoveOrderingPath: RUNTIME_MOVE_ORDERING_PROFILE ? path.join(args.outputRoot, 'shared', 'active-move-ordering-profile.json') : null,
      activeTuplePath: RUNTIME_TUPLE_RESIDUAL_PROFILE ? path.join(args.outputRoot, 'shared', 'active-tuple-residual-profile.json') : null,
      activeMpcPath: RUNTIME_MPC_PROFILE ? path.join(args.outputRoot, 'shared', 'active-mpc-profile.json') : null,
