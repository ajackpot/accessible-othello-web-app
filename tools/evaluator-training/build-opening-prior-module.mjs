...
function printUsage() {
  ...
  node ${toolPath} \
    ...
    [--format compact|factorized|expanded] [--hash-encoding hex|decimal] \
...
- 기본값은 factorized runtime 형식이며, JSON 학습 산출물을 정적 웹 앱용 모듈로 더 작게 내보냅니다.
- 필요하면 --format compact 또는 --format expanded를 지정할 수 있습니다.
`);
}
...
function createProfileSummary(profile) {
  ...
  const derivedMoveCount = positions.reduce((sum, position) => {
    if (Array.isArray(position)) {
      return sum + Math.max(0, Math.floor((position.length - 3) / 3));
    }
    return sum + (Array.isArray(position?.moves) ? position.moves.length : 0);
  }, 0);
  const runtimePositionCount = profile?.runtime?.positionCount ?? positions.length;
  const runtimeMoveCount = profile?.runtime?.moveCount ?? derivedMoveCount;
  ...
  return {
    ...
    positionCount: positions.length > 0 ? positions.length : runtimePositionCount,
    moveCount: positions.length > 0 ? derivedMoveCount : runtimeMoveCount,
...
const moduleFormat = typeof args.format === 'string' ? args.format : 'factorized';
const moduleOptions = {
  moduleFormat,
