...
import {
  PATTERN_BANK_LAYOUT_LIBRARY,
  resolvePatternBankProfile,
} from './pattern-bank-lib.mjs';
...
function builtinPatternBankLayoutNameForModule(layout) {
  ...
  const builtin = PATTERN_BANK_LAYOUT_LIBRARY[layoutName];
  if (!builtin || !Array.isArray(layout.basePatterns) || builtin.basePatterns.length !== layout.basePatterns.length) {
    return null;
  }
  for (let patternIndex = 0; patternIndex < builtin.basePatterns.length; patternIndex += 1) {
    const builtinPattern = builtin.basePatterns[patternIndex];
    const sourcePattern = layout.basePatterns[patternIndex];
    if (builtinPattern.key !== sourcePattern?.key || builtinPattern.placements.length !== sourcePattern?.placements?.length) {
      return null;
    }
    for (let placementIndex = 0; placementIndex < builtinPattern.placements.length; placementIndex += 1) {
      const builtinPlacement = builtinPattern.placements[placementIndex];
      const sourcePlacement = sourcePattern.placements[placementIndex];
      if (!Array.isArray(sourcePlacement) || builtinPlacement.length !== sourcePlacement.length) {
        return null;
      }
      for (let squareIndex = 0; squareIndex < builtinPlacement.length; squareIndex += 1) {
        if (builtinPlacement[squareIndex] !== sourcePlacement[squareIndex]) {
          return null;
        }
      }
    }
...
export function sanitizePatternBankProfileForModule(profile) {
  const resolved = resolvePatternBankProfile(profile);
  return resolved ? Object.freeze(JSON.parse(JSON.stringify(resolved))) : null;
}

function resolvePatternBankProfileStackForModule(profiles) {
  if (!profiles) {
    return [];
  }
...
export function sanitizePatternBankProfileStackForModule(profiles) {
  const resolved = resolvePatternBankProfileStackForModule(profiles);
  return resolved.length > 0
    ? Object.freeze(resolved.map((profile) => Object.freeze(JSON.parse(JSON.stringify(profile)))))
    : null;
}
...
function normalizeGeneratedProfilesModuleFormat(value) {
  ...
  if (normalized === 'factorized' || normalized === 'factorised') {
    return 'factorized';
  }
  return 'compact';
}

function encodeBase64FromUint8Array(bytes) {
...
function appendUnsignedVarint(target, value) {
  ...
  target.push(remaining & 0x7f);
}

function appendSignedVarint(target, value) {
  const normalized = Math.trunc(Number(value) || 0);
  const zigZag = normalized >= 0
    ? normalized * 2
    : ((-normalized) * 2) - 1;
  appendUnsignedVarint(target, zigZag);
}

function encodeUnsignedVarintArray(values) {
  const target = [];
  for (const value of values) {
    appendUnsignedVarint(target, value);
  }
  return Uint8Array.from(target);
}

function encodeSignedVarintArray(values) {
  const target = [];
  for (const value of values) {
    appendSignedVarint(target, value);
  }
  return Uint8Array.from(target);
}

function encodeFloat32ArrayBytes(values) {
  const buffer = Buffer.allocUnsafe(Math.max(0, values.length) * 4);
  for (let index = 0; index < values.length; index += 1) {
    buffer.writeFloatLE(Math.fround(Number(values[index] ?? 0)), index * 4);
  }
  return buffer;
}

function openingPriorHashStringToBytes(stateHash, hashEncoding = 'hex') {
...
function normalizeOpeningPriorHashEncoding(value, moduleFormat = 'compact') {
  ...
  return moduleFormat === 'expanded' ? 'decimal' : 'hex';
}
...
function compactPatternBankProfileForModule(profile) {
  const resolved = sanitizePatternBankProfileForModule(profile);
  if (!resolved) {
    return null;
  }

  const builtinLayoutName = builtinPatternBankLayoutNameForModule(resolved.layout);
  const compactLayout = builtinLayoutName
    ? null
    : Object.freeze({
      ...(typeof resolved.layout.name === 'string' && resolved.layout.name.trim() !== ''
        ? { name: resolved.layout.name }
        : {}),
      basePatterns: Object.freeze(resolved.layout.basePatterns.map((pattern) => Object.freeze({
        ...(typeof pattern.key === 'string' && pattern.key.trim() !== '' ? { key: pattern.key } : {}),
        placements: Object.freeze(pattern.placements.map((placement) => Object.freeze([...placement]))),
      }))),
    });
  ...
  return Object.freeze({
    ...
    trainedBuckets: Object.freeze(resolved.trainedBuckets.map((bucket) => Object.freeze({
      ...
      scale: bucket.scale,
      bias: bucket.bias,
      patternWeights: Object.freeze(bucket.patternWeights.map((table) => Object.freeze([...table]))),
    }))),
  });
}

function compactPatternBankProfileStackForModule(profiles) {
  const resolved = sanitizePatternBankProfileStackForModule(profiles);
  return resolved ? Object.freeze(resolved.map((profile) => compactPatternBankProfileForModule(profile))) : null;
}

function factorizedPatternBankProfileForModule(profile) {
  const resolved = sanitizePatternBankProfileForModule(profile);
  if (!resolved) {
    return null;
  }
  ...
  const tableNonZeroCounts = [];
  const encodedIndexBytes = [];
  const encodedValues = [];
  for (const bucket of resolved.trainedBuckets) {
    for (const table of bucket.patternWeights) {
      ...
      for (let entryIndex = 0; entryIndex < table.length; entryIndex += 1) {
        const value = Number(table[entryIndex] ?? 0);
        if (!Number.isFinite(value) || value === 0) {
          continue;
        }
        const delta = isFirstEntry ? entryIndex : (entryIndex - previousIndex);
        appendUnsignedVarint(encodedIndexBytes, delta);
        encodedValues.push(Math.fround(value));
        previousIndex = entryIndex;
        isFirstEntry = false;
        nonZeroCount += 1;
      }
      tableNonZeroCounts.push(nonZeroCount);
    }
  }

  return Object.freeze({
    version: resolved.version,
    format: 'factorized-sparse-v1',
    name: resolved.name,
    ...
    factorized: Object.freeze({
      indexEncoding: 'delta-uvarint-base64',
      valueEncoding: 'f32le-base64',
      tableNonZeroCounts: Object.freeze(tableNonZeroCounts),
      indicesBase64: encodeBase64FromUint8Array(Uint8Array.from(encodedIndexBytes)),
      valuesBase64: encodeBase64FromUint8Array(encodeFloat32ArrayBytes(encodedValues)),
    }),
  });
}

function factorizedPatternBankProfileStackForModule(profiles) {
  const resolved = sanitizePatternBankProfileStackForModule(profiles);
  return resolved ? Object.freeze(resolved.map((profile) => factorizedPatternBankProfileForModule(profile))) : null;
}
...
function factorizedOpeningPriorProfileForModule(profile, {
  hashEncoding = 'hex',
  maxPly = null,
  minPositionCount = 0,
  minMoveCount = 0,
  ...
  const compactProfile = compactOpeningPriorProfileForModule(profile, {
    hashEncoding,
    maxPly,
    minPositionCount,
    minMoveCount,
  ...
  for (const position of compactProfile.positions) {
    const stateHashBytes = openingPriorHashStringToBytes(position[0], compactProfile.hashEncoding);
    hashLengths.push(stateHashBytes.length);
    hashBytes.push(...stateHashBytes);
    plys.push(Math.max(0, Math.trunc(Number(position[1] ?? 0))));
    totalCounts.push(Math.max(0, Math.trunc(Number(position[2] ?? 0))));
    const candidateCount = Math.max(0, Math.floor((position.length - 3) / 3));
    candidateCounts.push(candidateCount);
    for (let index = 3; index + 2 < position.length; index += 3) {
      moveIndices.push(Math.max(0, Math.trunc(Number(position[index] ?? 0))));
      moveCounts.push(Math.max(0, Math.trunc(Number(position[index + 1] ?? 0))));
      moveScores.push(Math.trunc(Number(position[index + 2] ?? 0)));
    }
  }

  return {
    version: compactProfile.version,
    format: 'factorized-binary-varint-v1',
    hashEncoding: compactProfile.hashEncoding,
    name: compactProfile.name,
    description: compactProfile.description,
    symmetry: compactProfile.symmetry,
    ...(Object.hasOwn(compactProfile, 'stage') ? { stage: compactProfile.stage } : {}),
    ...(Object.hasOwn(compactProfile, 'source') ? { source: compactProfile.source } : {}),
    ...(Object.hasOwn(compactProfile, 'options') ? { options: compactProfile.options } : {}),
    ...(Object.hasOwn(compactProfile, 'diagnostics') ? { diagnostics: compactProfile.diagnostics } : {}),
    runtime: compactProfile.runtime,
    factorized: {
      hashBytesBase64: encodeBase64FromUint8Array(Uint8Array.from(hashBytes)),
      hashLengthsBase64: encodeBase64FromUint8Array(Uint8Array.from(hashLengths)),
      plysBase64: encodeBase64FromUint8Array(Uint8Array.from(plys)),
      candidateCountsBase64: encodeBase64FromUint8Array(Uint8Array.from(candidateCounts)),
      totalCountsBase64: encodeBase64FromUint8Array(encodeUnsignedVarintArray(totalCounts)),
      moveIndicesBase64: encodeBase64FromUint8Array(Uint8Array.from(moveIndices)),
      moveCountsBase64: encodeBase64FromUint8Array(encodeUnsignedVarintArray(moveCounts)),
      moveScoresBase64: encodeBase64FromUint8Array(encodeSignedVarintArray(moveScores)),
    },
  };
}
...
export function sanitizeOpeningPriorProfileForModule(profile, {
  ...
  if (normalizedModuleFormat === 'factorized') {
    return factorizedOpeningPriorProfileForModule(profile, {
...
export function renderGeneratedProfilesModule({
  ...
  const useExpandedProfiles = normalizedModuleFormat === 'expanded';
  const useFactorizedPatternBankProfiles = normalizedModuleFormat === 'factorized';
  const normalizedEvaluationProfile = useExpandedProfiles
    ? sanitizeEvaluationProfileForModule(evaluationProfile)
    : compactEvaluationProfileForModule(evaluationProfile);
  const normalizedMoveOrderingProfile = useExpandedProfiles
    ? sanitizeMoveOrderingProfileForModule(moveOrderingProfile)
    : compactMoveOrderingProfileForModule(moveOrderingProfile);
  const normalizedTupleResidualProfile = useExpandedProfiles
    ? sanitizeTupleResidualProfileForModule(tupleResidualProfile)
    : compactTupleResidualProfileForModule(tupleResidualProfile);
  const normalizedMpcProfile = useExpandedProfiles
    ? sanitizeMpcProfileForModule(mpcProfile)
    : compactMpcProfileForModule(mpcProfile);
  const normalizedPatternBankProfiles = useExpandedProfiles
    ? sanitizePatternBankProfileStackForModule(patternBankProfiles)
    : (useFactorizedPatternBankProfiles
      ? factorizedPatternBankProfileStackForModule(patternBankProfiles)
      : compactPatternBankProfileStackForModule(patternBankProfiles));
  const normalizedMoveOrderingPatternBankProfiles = useExpandedProfiles
    ? sanitizePatternBankProfileStackForModule(moveOrderingPatternBankProfiles)
    : (useFactorizedPatternBankProfiles
      ? factorizedPatternBankProfileStackForModule(moveOrderingPatternBankProfiles)
      : compactPatternBankProfileStackForModule(moveOrderingPatternBankProfiles));
  ...
  const patternBankSerialized = normalizedPatternBankProfiles ? JSON.stringify(normalizedPatternBankProfiles, null, indentation) : 'null';
  const moveOrderingPatternBankSerialized = normalizedMoveOrderingPatternBankProfiles ? JSON.stringify(normalizedMoveOrderingPatternBankProfiles, null, indentation) : 'null';
...
const GENERATED_PATTERN_BANK_PROFILES = ${patternBankSerialized === 'null' ? 'null' : `Object.freeze(${patternBankSerialized})`};
const GENERATED_MOVE_ORDERING_PATTERN_BANK_PROFILES = ${moveOrderingPatternBankSerialized === 'null' ? 'null' : `Object.freeze(${moveOrderingPatternBankSerialized})`};

export { GENERATED_EVALUATION_PROFILE, GENERATED_MOVE_ORDERING_PROFILE, GENERATED_TUPLE_RESIDUAL_PROFILE, GENERATED_MPC_PROFILE, GENERATED_PATTERN_BANK_PROFILES, GENERATED_MOVE_ORDERING_PATTERN_BANK_PROFILES };
...
const GENERATED_PATTERN_BANK_PROFILES=${patternBankSerialized};
const GENERATED_MOVE_ORDERING_PATTERN_BANK_PROFILES=${moveOrderingPatternBankSerialized};
export{GENERATED_EVALUATION_PROFILE,GENERATED_MOVE_ORDERING_PROFILE,GENERATED_TUPLE_RESIDUAL_PROFILE,GENERATED_MPC_PROFILE,GENERATED_PATTERN_BANK_PROFILES,GENERATED_MOVE_ORDERING_PATTERN_BANK_PROFILES};
export default GENERATED_EVALUATION_PROFILE;
`;
}
...
export async function loadGeneratedProfilesModuleIfPresent(filePath) {
  ...
  return {
    ...
    patternBankProfiles: imported.GENERATED_PATTERN_BANK_PROFILES ?? imported.GENERATED_PATTERN_BANK_PROFILE ?? null,
    moveOrderingPatternBankProfiles: imported.GENERATED_MOVE_ORDERING_PATTERN_BANK_PROFILES ?? imported.GENERATED_MOVE_ORDERING_PATTERN_BANK_PROFILE ?? null,
  };
}

export function createEvaluatorForProfile(profile, {
  tupleResidualProfile = null,
  patternBankProfiles = null,
} = {}) {
  return new Evaluator({ evaluationProfile: profile, tupleResidualProfile, patternBankProfiles });
}
