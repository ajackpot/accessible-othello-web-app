...
function printUsage() {
  const toolPath = displayTrainingToolPath('install-tuple-residual-profile.mjs');
  ...
  const summaryJsonPath = displayProjectPath('benchmarks', 'tuple_residual_install_summary.json');
  console.log(`Usage:
  node ${toolPath} \
...
동작:
- tuple residual JSON을 앱용 learned-eval-profile.generated.js에 설치합니다.
- evaluation / move-ordering / MPC slot은 별도 JSON을 주지 않으면 현재 활성 module 값을 그대로 보존합니다.
- generated.js를 따로 첨부받지 못했더라도, tuple JSON만으로 현재 repo의 app 모듈을 안전하게 갱신할 수 있습니다.
`);
}
...
const tupleResidualProfile = sanitizeTupleResidualProfileForModule(loadJsonFileIfPresent(tupleJsonPath));
const mpcProfile = sanitizeMpcProfileForModule(
  loadJsonFileIfPresent(args['mpc-json']) ?? RUNTIME_MPC_PROFILE ?? null,
);

if (!tupleResidualProfile) {
  throw new Error(`읽을 수 있는 tuple residual profile JSON이 없습니다: ${tupleJsonPath}`);
}
...
const summary = {
  ...
  tupleLayoutName: tupleResidualProfile?.layout?.name ?? null,
  tupleCount: tupleResidualProfile?.layout?.tupleCount ?? null,
  trainedBucketCount: Array.isArray(tupleResidualProfile?.trainedBuckets) ? tupleResidualProfile.trainedBuckets.length : 0,
  mpcProfileName: mpcProfile?.name ?? null,
};

console.log(`Installed tuple residual profile into ${writtenPath}`);
...
console.log(`  tuple layout    : ${tupleResidualProfile?.layout?.name ?? 'null'} (${tupleResidualProfile?.layout?.tupleCount ?? 0} tuples)`);
