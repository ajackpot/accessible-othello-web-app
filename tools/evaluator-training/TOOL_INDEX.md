...
특징:
- config JSON 지원
- shared installed-runtime snapshot 저장
- candidate별 `candidate-status.json`
- `suite-summary.json`
- `--resume`, `--plan-only`, `--continue-on-error`
...
특징:
- config JSON 지원
- shared installed-runtime evaluation/move-ordering/tuple/MPC snapshot 저장
...
명시하지 않은 slot은 현재 설치 런타임 module 값을 보존한 채 새 generated module을 만듭니다.
...
현재 설치 런타임 evaluation / move-ordering / MPC를 보존하면서 tuple residual만 교체할 때 씁니다.

### `install-mpc-profile.mjs` / `.bat`
현재 설치 런타임 evaluation / move-ordering / tuple residual을 보존하면서 MPC slot만 교체할 때 씁니다.
