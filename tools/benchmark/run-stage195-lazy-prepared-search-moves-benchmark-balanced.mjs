...
const DEFAULT_OUTPUT_PATH = path.join(
  repoRoot,
  'benchmarks',
  'stage195_lazy_prepared_search_moves_benchmark_balanced_20260421.json',
);
...
function runMoveGenerationMicroSection() {
  ...
  // Warm both builders once to reduce first-run bias.
  runBuilder('warm_baseline_eager', true);
  runBuilder('warm_candidate_lazy', false);

  const baseline = runBuilder('baseline_eager', true);
  const candidate = runBuilder('candidate_lazy', false);
...
function runBalancedPair(state, baselineOptions, candidateOptions, repetitions) {
  ...
  // Warm both lanes before measurement.
