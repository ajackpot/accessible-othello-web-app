...
function parseArgs(argv) {
  const parsed = { section: null, output: null };
  ...
  if (!parsed.output) {
    parsed.output = path.join(repoRoot, 'benchmarks', `stage196_${parsed.section}.json`);
  }
  return parsed;
}

function withToggle(options, tokenizedPreparedSearchMoveCore) {
  return {
    ...options,
...
function runMoveGenerationMicroSection({ label, eager }) {
  const corpus = buildMicroCorpus();

  const runBuilder = (name, coreVariant) => {
    ...
    for (let repetition = 0; repetition < MICRO_REPETITIONS; repetition += 1) {
      for (const entry of corpus) {
        ...
        const moves = listPreparedSearchMoves(player, opponent, { eager, coreVariant });
  ...
  runBuilder('warm_baseline_legacy', PREPARED_SEARCH_MOVE_CORE_VARIANTS.LEGACY);
  runBuilder('warm_candidate_tokenized', PREPARED_SEARCH_MOVE_CORE_VARIANTS.TOKENIZED);

  const baseline = runBuilder('baseline_legacy', PREPARED_SEARCH_MOVE_CORE_VARIANTS.LEGACY);
  const candidate = runBuilder('candidate_tokenized', PREPARED_SEARCH_MOVE_CORE_VARIANTS.TOKENIZED);
...
function runSectionByName(name) {
  switch (name) {
    case 'micro_eager':
      return runMoveGenerationMicroSection({ label: 'prepared_move_micro_eager', eager: true });
    case 'micro_lazy':
      return runMoveGenerationMicroSection({ label: 'prepared_move_micro_lazy', eager: false });
    case 'depth24':
      return runSearchSection({
...
async function main() {
  ...
  const output = runSectionByName(args.section);
  await fs.mkdir(path.dirname(args.output), { recursive: true });
  await fs.writeFile(args.output, `${JSON.stringify(output, null, 2)}\n`, 'utf8');
  console.log(JSON.stringify({
    outputPath: args.output,
    section: args.section,
    summary: output.summary ?? { elapsedRatioCandidateVsBaseline: output.elapsedRatioCandidateVsBaseline },
  }, null, 2));
}
