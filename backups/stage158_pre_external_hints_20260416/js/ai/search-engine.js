...
import {
  ...
  ACTIVE_MPC_PROFILE,
  ACTIVE_MOVE_ORDERING_PATTERN_BANK_PROFILES,
  ACTIVE_MOVE_ORDERING_PROFILE,
  ACTIVE_PATTERN_BANK_PROFILES,
...
function resolveOptionsFromInput(
  ...
  const edgePatternScale = sanitizeExperimentalNumber(engineOptions.edgePatternScale, resolved.edgePatternScale ?? 1, 0, 100, 3);
  const cornerPatternScale = sanitizeExperimentalNumber(engineOptions.cornerPatternScale, resolved.cornerPatternScale ?? 1, 0, 100, 3);
  ...
  return {
    ...
    patternBankProfiles: resolvePatternBankProfilesOption(
      engineOptions,
      resolved,
      ACTIVE_PATTERN_BANK_PROFILES ?? null,
    ),
    moveOrderingPatternBankProfiles: resolveMoveOrderingPatternBankProfilesOption(
      engineOptions,
      resolved,
      ACTIVE_MOVE_ORDERING_PATTERN_BANK_PROFILES ?? null,
    ),
    ...
    moveOrderingEdgePatternScale: sanitizeExperimentalNumber(
      engineOptions.moveOrderingEdgePatternScale,
      resolved.moveOrderingEdgePatternScale ?? edgePatternScale,
      0,
      100,
      3,
    ),
    moveOrderingCornerPatternScale: sanitizeExperimentalNumber(
      engineOptions.moveOrderingCornerPatternScale,
      resolved.moveOrderingCornerPatternScale ?? cornerPatternScale,
      0,
      100,
      3,
    ),
...
export class SearchEngine {
  ...
  getMpcRuntimeConfig() {
    const baseRuntime = this.compiledMpcProfile?.runtime ?? null;
    const runtimeOverrides = this.mpcStructureProfile?.runtimeOverrides ?? null;
  ...
  shouldSkipMpcByVolatility(state) {
    ...
    if (Number.isFinite(profile.volatilityMaxLegalMoves) && legalMoves.length > profile.volatilityMaxLegalMoves) {
      return true;
    }
    if (profile.volatilitySkipCornerAvailable && legalMoves.some((move) => CORNER_INDEX_SET.has(move.index))) {
      return true;
    }
    return false;
  }
    ...
    if (this.shouldRunOrderingShallowProbe(empties, depthRemaining)) {
      ...
      if (probeTopK > 0) {
        for (let index = 0; index < probeTopK; index += 1) {
          const move = moves[index];
          const childState = move.orderingOutcome ?? state.applyMoveFast(move.index, move.flips ?? null);
          if (!childState) {
            continue;
          }
  ...
  scoreMoveForOrdering(state, move, context) {
    ...
    if (outcome) {
      ...
      const potentialMobilityBonus = this.getOrderingPotentialMobilityBonus(childState, state.currentPlayer);
      if (potentialMobilityBonus !== 0) {
        this.stats.orderingPotentialMobilityBonuses += 1;
        score += potentialMobilityBonus;
      }

      const frontierBonus = this.getOrderingFrontierBonus(childState, state.currentPlayer);
      if (frontierBonus !== 0) {
