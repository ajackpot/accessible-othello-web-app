...
const teacherTupleResidualProfileInput = loadJsonFileOrExplicitNull(args['teacher-tuple-profile-json'], teacherGeneratedModule?.tupleResidualProfile ?? ACTIVE_TUPLE_RESIDUAL_PROFILE ?? null);
const teacherMpcProfileInput = loadJsonFileOrExplicitNull(args['teacher-mpc-profile-json'], teacherGeneratedModule?.mpcProfile ?? ACTIVE_MPC_PROFILE ?? null);
...
const teacherEvaluationProfile = teacherEvaluationProfileInput ?? teacherGeneratedModule?.evaluationProfile ?? ACTIVE_EVALUATION_PROFILE;
const teacherMoveOrderingProfile = teacherMoveOrderingProfileInput ?? teacherGeneratedModule?.moveOrderingProfile ?? ACTIVE_MOVE_ORDERING_PROFILE ?? null;
...
const moduleEvaluationProfile = moduleEvaluationProfileInput ?? moduleGeneratedModule?.evaluationProfile ?? teacherEvaluationProfile ?? ACTIVE_EVALUATION_PROFILE;
