...
const mpcProfile = sanitizeMpcProfileForModule(
  loadJsonFileIfPresent(args['mpc-json']) ?? ACTIVE_MPC_PROFILE ?? null,
);
const patternBankProfiles = sanitizePatternBankProfileStackForModule(
  loadJsonStackIfPresent(patternBankJsonPaths) ?? ACTIVE_PATTERN_BANK_PROFILES ?? null,
);
const moveOrderingPatternBankProfiles = sanitizePatternBankProfileStackForModule(
  loadJsonStackIfPresent(moveOrderingPatternBankJsonPaths) ?? ACTIVE_MOVE_ORDERING_PATTERN_BANK_PROFILES ?? null,
);
