...
const evaluationProfile = loadJsonFileIfPresent(args['evaluation-profile-json']) ?? generatedModule?.evaluationProfile ?? ACTIVE_EVALUATION_PROFILE;
const moveOrderingProfile = loadJsonFileIfPresent(args['move-ordering-profile-json']) ?? generatedModule?.moveOrderingProfile ?? ACTIVE_MOVE_ORDERING_PROFILE ?? null;
const tupleResidualProfile = loadJsonFileOrExplicitNull(args['tuple-profile-json'], generatedModule?.tupleResidualProfile ?? ACTIVE_TUPLE_RESIDUAL_PROFILE ?? null);
const mpcProfile = loadJsonFileOrExplicitNull(args['mpc-profile-json'], generatedModule?.mpcProfile ?? ACTIVE_MPC_PROFILE ?? null);
